...
# --- 2026-09 re-audit: credentials in a peer-authored source_hub ---------------------

def test_userinfo_is_stripped_from_a_peer_supplied_origin():
    """`source_hub` comes from the hub's federated catalogue, i.e. from a peer.

    `_normalise_origin` rebuilt the URL with `parsed.netloc` intact, so
    "http://alice:s3cr3t@peer.example/family" was fetched verbatim — httpx turns userinfo
    into an `Authorization: Basic` header — and then kept as the cache key. A well-known
    document is public by definition; there is nothing here to authenticate to. The sibling
    copy in aimarket-agent already stripped it, with the reasoning written out.
    """
    from aimarket_bridges.receipts import _normalise_origin

    assert _normalise_origin("http://alice:s3cr3t@peer.example/family") == (
        "http://peer.example/family"
    )
    assert _normalise_origin("https://u:p@peer.example") == "https://peer.example"
    # A bare @ with no credentials must not eat the host.
    assert _normalise_origin("https://peer.example/x") == "https://peer.example/x"


def test_a_rejected_scheme_does_not_log_the_credentials():
    """The one place a rejected origin's userinfo would reach a log file."""
    import logging

    from aimarket_bridges import receipts
    ...
    handler = _Handler()
    receipts.logger.addHandler(handler)
    try:
        assert receipts._normalise_origin("ftp://alice:s3cr3t@peer.example") == ""
    finally:
        receipts.logger.removeHandler(handler)

    joined = " ".join(records)
    assert "s3cr3t" not in joined, f"credentials logged: {joined}"
    assert "ftp" in joined, "the warning should still say what was rejected"
