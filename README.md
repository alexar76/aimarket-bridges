...
> 🌐 **English** · [Русский](docs/ru.md) · [Español](docs/es.md) · [Français](docs/fr.md) · [中文](docs/zh.md) · [Glossary](https://github.com/alexar76/aicom/blob/main/docs/localization-glossary.md)
...
The core has no framework dependency. Install only the one you use: their pydantic floors have
collided before, and a bridge has no business dragging one framework into another's
environment. (Measured 2026-08-24: crewai 1.15.17, autogen-core 0.7.5 and agent-framework-core
1.15.0 do currently resolve together on pydantic 2.12.5, which is how CI runs all four suites
in one job.)
...
| Microsoft Agent Framework | `aimarket_bridges.agent_framework` | `list[agent_framework.FunctionTool]` |
...
Written against langchain-core 1.5.2, langgraph 1.2.10, crewai 1.15.8, autogen-core 0.7.5 and
agent-framework-core 1.15.0, each verified by introspection rather than from documentation.
They disagree about how a tool declares its arguments, which is most of what this package
absorbs:
...
- **agent-framework** takes either, and only one of the two works here: given a pydantic model
  it advertises the by-alias schema but validates the un-aliased dump against it, so
  `fourier.verify@v1` — whose argument really is called `lambda` — can never be called
  (`TypeError: Missing required argument(s): lambda`). The adapter hands over the hub's own
  schema instead, which the framework supports directly and which the model then reads
  verbatim.
