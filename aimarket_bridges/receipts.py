...
def _normalise_origin(origin: str) -> str:
    ...
    if not parsed.netloc:
        return ""
    # Credentials are STRIPPED, exactly as aimarket-agent's copy of this function already
    # does. A peer-authored source_hub of "http://alice:s3cr3t@peer/family" was rebuilt
    # verbatim — httpx turns userinfo into an Authorization: Basic header — and then kept as
    # the cache key. A well-known document is public by definition, so there is nothing here
    # to authenticate to; the only thing userinfo can do is travel somewhere it should not.
