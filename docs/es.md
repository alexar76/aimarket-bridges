...
herramientas nativas dentro de LangChain/LangGraph, CrewAI, AutoGen y Microsoft Agent Framework.
...
flowchart TB
    subgraph consumer["Proceso del consumidor"]
        agent["Agente<br/>(LangGraph · CrewAI<br/>AutoGen · Agent Framework)"]
        subgraph bridge["aimarket-bridges"]
            adapter["Adaptador de framework<br/>langchain.py · crewai.py<br/>autogen.py · agent_framework.py"]
...
| Adaptadores | `langchain.py`, `crewai.py`, `autogen.py`, `agent_framework.py` | uno por framework |
...
flowchart TB
    ...
    cap -->|"el esquema tal cual"| af["agent-framework-core 1.15.0<br/>FunctionTool(input_model=schema)<br/>async invoke()"]
    ...
    style af fill:#0f4c4a,stroke:#5eead4,color:#ecfeff
```

Cada afirmación de aquí abajo se midió por introspección contra la versión instalada, no se tomó
de la documentación — los cuatro habían avanzado más allá de lo que la documentación daba a
entender.
...
**agent-framework-core 1.15.0** acepta tanto un modelo pydantic como un JSON Schema en crudo, y
aquí solo funciona el esquema. Con un modelo anuncia el esquema con alias y luego valida contra
él el volcado *sin* alias, así que `fourier.verify@v1` no se puede llamar por esa puerta:
`TypeError: Missing required argument(s): lambda`. Con el esquema del hub valida los argumentos
obligatorios y los tipos escalares antes de la llamada (nada de eso cuesta dinero), pero reenvía
las propiedades no declaradas, que descarta el propio adaptador. Las herramientas síncronas las
lleva a `asyncio.to_thread` — el executor por defecto compartido del bucle —, por eso el
adaptador entrega una corrutina y usa su propio pool acotado.
...
Esa misma trampa es la razón por la que el adaptador de Agent Framework renuncia del todo al
modelo pydantic: agent-framework 1.15.0 anuncia `model_json_schema()` (con alias, `lambda`) pero
valida contra él `model_dump(exclude_unset=True)` (sin alias, `lambda_`), de modo que el viaje de
ida y vuelta se rompe dentro del framework, no en el puente. Allí la solución es entregar el
esquema del hub y no construir modelo alguno.
...
```bash
pip install "aimarket-bridges[agent-framework]"
```

Instala solo el extra que uses: el paquete no mantiene ningún framework entre sus dependencias,
porque sus mínimos de pydantic ya han chocado y un puente no tiene por qué arrastrar un
framework al entorno de otro. Medido el 2026-08-24: crewai 1.15.17, autogen-core 0.7.5 y
agent-framework-core 1.15.0 sí se resuelven juntos hoy sobre pydantic 2.12.5, que es como CI
ejecuta las cuatro suites en un solo job — por eso este paquete no mantiene ningún framework entre sus propias
dependencias.
...
from aimarket_bridges.agent_framework import aimarket_tools

tools = aimarket_tools("https://modelmarket.dev", budget_usd=0.50)
agent = ChatAgent(chat_client=your_client, instructions="Buy only what you need.", tools=tools)
```

Para llamar a una herramienta directamente: `invoke` es asíncrono y recibe los argumentos como
un mapa:
...
tool = next(t for t in tools if t.capability.capability_id == "sortes.draw@v1")
contents = asyncio.run(tool.invoke(arguments={"alpha": "my-seed"}))

contents[0].text                    # exactamente lo que lee el modelo
tool.last_result.output             # la respuesta de la propia capability
tool.last_result.receipt_verified   # True
```

Este adaptador entrega a `FunctionTool` el **JSON Schema** del hub, no un modelo pydantic. Con
un modelo, agent-framework 1.15.0 anuncia el esquema con alias y luego valida contra él el
volcado sin alias, de modo que una capability cuyo argumento se llama `lambda` —
`fourier.verify@v1` es una — no se puede llamar nunca:
`TypeError: Missing required argument(s): lambda`. Entregar el esquema elimina ese rodeo por
completo, y el modelo lee el contrato del hub tal cual.

Dos consecuencias que conviene conocer. El framework rechaza un argumento obligatorio ausente y
un tipo escalar equivocado **antes** de la llamada, así que ninguno de los dos cuesta dinero. Lo
que **no** rechaza es una propiedad que el esquema nunca declaró: esas las descarta el propio
adaptador, porque enviar un argumento inventado significa pagar por una llamada que la
capability va a rechazar.
...
Las dos versiones de pydantic de arriba son históricas: medido de nuevo el 2026-08-24, crewai
1.15.17, autogen-core 0.7.5 y agent-framework-core 1.15.0 se resuelven juntos sobre pydantic
2.12.5, y CI ejecuta ahora las cuatro suites — 582 pruebas — en un solo entorno.
