...
AutoGen 和 Microsoft Agent Framework 里表现为原生工具。
...
flowchart TB
    subgraph consumer["消费方进程"]
        agent["智能体<br/>(LangGraph · CrewAI<br/>AutoGen · Agent Framework)"]
        subgraph bridge["aimarket-bridges"]
            adapter["框架适配器<br/>langchain.py · crewai.py<br/>autogen.py · agent_framework.py"]
...
| 适配器 | `langchain.py`、`crewai.py`、`autogen.py`、`agent_framework.py` | 每个文件对应一个框架 |
...
flowchart TB
    ...
    cap -->|"原样交出 schema"| af["agent-framework-core 1.15.0<br/>FunctionTool(input_model=schema)<br/>async invoke()"]
...
下面每一条陈述，都是针对已安装版本做内省实测得到的，不是从文档里抄来的——四个框架都已经跑到
文档所暗示的位置之外了。
...
**agent-framework-core 1.15.0** 既接受 pydantic 模型，也接受原始 JSON Schema——但这里只有 schema
这条路走得通。给它模型，它会按别名公布 schema，再拿*去掉别名*的 dump 去校验，于是 `fourier.verify@v1`
从这扇门根本调不动：`TypeError: Missing required argument(s): lambda`。交给它 hub 的 schema 之后，
它会在调用前校验必填参数与标量类型（这两种都不花钱），但会把未声明的属性照样转发——那些由适配器
自己丢掉。同步工具它会送进 `asyncio.to_thread`，也就是事件循环共享的默认 executor，所以适配器改为
交出协程，并使用自己的有界线程池。
...
正是同一个陷阱，让 Agent Framework 适配器干脆不用 pydantic 模型：agent-framework 1.15.0 对外公布的是
`model_json_schema()`（带别名，`lambda`），拿去校验的却是 `model_dump(exclude_unset=True)`（不带别名，
`lambda_`）——也就是说这趟往返是在框架内部断的，不在桥这边。那里的解法只有一个：把 hub 的 schema 原样
交出去，根本不建模型。
...
只装你要用的那一个 extra：本包自己不把任何框架写进依赖，因为它们的 pydantic 下限撞过车，而桥没有理由
把一个框架拖进另一个框架的环境。2026-08-24 实测：crewai 1.15.17、autogen-core 0.7.5 与
agent-framework-core 1.15.0 目前可以在 pydantic 2.12.5 上一起解析——CI 也正是这样在一个 job 里跑完
四套测试——
正因如此，本包自己的依赖里不放任何框架。
...
contents[0].text                    # 模型读到的正是这段文本
tool.last_result.output             # capability 自己的答案
tool.last_result.receipt_verified   # True
```

这个适配器交给 `FunctionTool` 的是 hub 的 **JSON Schema**，不是 pydantic 模型。若给它模型，
agent-framework 1.15.0 会按别名公布 schema，却拿去掉别名的 dump 去校验，于是参数名叫 `lambda` 的
capability（`fourier.verify@v1` 就是一个）根本无法调用：
`TypeError: Missing required argument(s): lambda`。直接交出 schema 免掉了这一圈，模型读到的就是
hub 自己的契约。

有两点值得知道。缺少必填参数、标量类型不对，框架都会在调用**之前**拒绝，所以这两种都不花钱。但
schema 里没声明过的属性它并**不**拒绝——那些由适配器自己丢掉：把凭空编出来的参数发过去，等于为一次
capability 必然拒绝的调用付钱。
...
适配器是针对 crewai **1.15.8** 写的，而在 1.15.9 上也能通过，这才是有用的那个事实。上面那两个
pydantic 版本属于历史记录：2026-08-24 重新实测，crewai 1.15.17、autogen-core 0.7.5 与
agent-framework-core 1.15.0 可以一起解析到 pydantic 2.12.5，CI 现在在同一个环境里跑完四套测试，共
582 个。
