...
outils natifs dans LangChain/LangGraph, CrewAI, AutoGen et Microsoft Agent Framework.
...
flowchart TB
    subgraph consumer["Processus consommateur"]
        agent["Agent<br/>(LangGraph · CrewAI<br/>AutoGen · Agent Framework)"]
        subgraph bridge["aimarket-bridges"]
            adapter["Adaptateur de framework<br/>langchain.py · crewai.py<br/>autogen.py · agent_framework.py"]
...
| Adaptateurs | `langchain.py`, `crewai.py`, `autogen.py`, `agent_framework.py` | un framework chacun |
...
flowchart TB
    ...
    cap -->|"le schéma tel quel"| af["agent-framework-core 1.15.0<br/>FunctionTool(input_model=schema)<br/>async invoke()"]
...
tirée de la documentation — les quatre avaient dépassé ce que leurs docs laissaient entendre.
...
**agent-framework-core 1.15.0** accepte aussi bien un modèle pydantic qu'un JSON Schema brut —
et seul le schéma fonctionne ici. Avec un modèle, il annonce le schéma avec alias puis valide
contre lui le dump *sans* alias : `fourier.verify@v1` devient inappelable par cette porte
(`TypeError: Missing required argument(s): lambda`). Avec le schéma du hub, il valide les
arguments obligatoires et les types scalaires avant l'appel (rien de tout cela ne coûte
d'argent), mais transmet les propriétés non déclarées — que l'adaptateur écarte lui-même. Les
outils synchrones partent dans `asyncio.to_thread`, l'executor par défaut partagé de la boucle ;
l'adaptateur fournit donc une coroutine et utilise son propre pool borné.
...
C'est exactement ce piège qui fait que l'adaptateur Agent Framework renonce complètement au
modèle pydantic : agent-framework 1.15.0 annonce `model_json_schema()` (avec alias, `lambda`)
mais valide contre lui `model_dump(exclude_unset=True)` (sans alias, `lambda_`) — l'aller-retour
casse donc à l'intérieur du framework, pas dans le pont. Là, le remède est de transmettre le
schéma du hub et de ne construire aucun modèle.
...
N'installez que l'extra que vous utilisez : ce paquet ne garde aucun framework dans ses propres
dépendances, parce que leurs planchers pydantic se sont déjà heurtés et qu'un pont n'a pas à
entraîner un framework dans l'environnement d'un autre. Mesuré le 2026-08-24 : crewai 1.15.17,
autogen-core 0.7.5 et agent-framework-core 1.15.0 se résolvent aujourd'hui ensemble sur
pydantic 2.12.5, ce qui est la façon dont la CI exécute les quatre suites dans un seul job — c'est pour cette raison que ce paquet
...
Pour appeler un outil directement : `invoke` est asynchrone et prend les arguments sous forme de
mapping :
...
contents[0].text                    # exactement ce que lit le modèle
tool.last_result.output             # la réponse de la capability elle-même
tool.last_result.receipt_verified   # True
```

Cet adaptateur transmet à `FunctionTool` le **JSON Schema** du hub, et non un modèle pydantic.
Avec un modèle, agent-framework 1.15.0 annonce le schéma avec alias puis valide contre lui le
dump sans alias : une capability dont l'argument s'appelle `lambda` — `fourier.verify@v1` en est
une — ne peut alors jamais être appelée
(`TypeError: Missing required argument(s): lambda`). Transmettre le schéma supprime tout ce
détour, et le modèle lit le contrat du hub tel quel.

Deux conséquences à connaître. Le framework refuse un argument obligatoire manquant et un type
scalaire erroné **avant** l'appel : ni l'un ni l'autre ne coûte d'argent. En revanche il ne
refuse **pas** une propriété que le schéma n'a jamais déclarée — celles-là, l'adaptateur les
écarte lui-même, car envoyer un argument inventé revient à payer un appel que la capability
refusera.
...
fait utile. Les deux versions de pydantic ci-dessus sont historiques : mesuré à nouveau le
2026-08-24, crewai 1.15.17, autogen-core 0.7.5 et agent-framework-core 1.15.0 se résolvent
ensemble sur pydantic 2.12.5, et la CI exécute désormais les quatre suites — 582 tests — dans un
seul environnement.
