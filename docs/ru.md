...
инструментами внутри LangChain/LangGraph, CrewAI, AutoGen и Microsoft Agent Framework.
...
flowchart TB
    subgraph consumer["Процесс потребителя"]
        agent["Агент<br/>(LangGraph · CrewAI<br/>AutoGen · Agent Framework)"]
        subgraph bridge["aimarket-bridges"]
            adapter["Адаптер фреймворка<br/>langchain.py · crewai.py<br/>autogen.py · agent_framework.py"]
...
| Адаптеры | `langchain.py`, `crewai.py`, `autogen.py`, `agent_framework.py` | по одному фреймворку на каждый |
...
flowchart TB
    ...
    cap -->|"схема как опубликована"| af["agent-framework-core 1.15.0<br/>FunctionTool(input_model=schema)<br/>async invoke()"]
...
— все четыре ушли дальше того, что следовало из их документации.
...
**agent-framework-core 1.15.0** принимает и pydantic-модель, и сырую JSON Schema — работает
здесь только схема. С моделью он публикует схему по алиасам, а проверяет по ней дамп *без*
алиасов, поэтому `fourier.verify@v1` через эту дверь не вызвать вообще:
`TypeError: Missing required argument(s): lambda`. Получив схему хаба, он проверяет обязательные
аргументы и скалярные типы до вызова (это не стоит денег), но пропускает незаявленные свойства —
их отбрасывает сам адаптер. Синхронные инструменты он уводит в `asyncio.to_thread`, то есть в
общий дефолтный executor цикла, поэтому адаптер отдаёт корутину и работает со своим ограниченным
пулом.
...
Ровно из-за этой ловушки адаптер Agent Framework вообще отказывается от pydantic-модели:
agent-framework 1.15.0 публикует `model_json_schema()` (с алиасом, `lambda`), а проверяет по ней
`model_dump(exclude_unset=True)` (без алиаса, `lambda_`) — то есть круг разрывается внутри самого
фреймворка, а не в мосту. Там лечение одно: отдать схему хаба и не строить модель вовсе.
...
Устанавливайте только тот extra, которым пользуетесь: сам пакет не держит ни один фреймворк в
зависимостях, потому что их нижние границы pydantic уже сталкивались, а мосту незачем тянуть
один фреймворк в окружение другого. Измерено 2026-08-24: crewai 1.15.17, autogen-core 0.7.5 и
agent-framework-core 1.15.0 сегодня разрешаются вместе на pydantic 2.12.5 — именно так CI и
гоняет все четыре набора тестов в одной job — именно поэтому этот пакет не держит ни один
...
Вызвать инструмент напрямую: `invoke` асинхронный и принимает аргументы отображением:
...
contents[0].text                    # ровно то, что прочитает модель
tool.last_result.output             # собственный ответ capability
tool.last_result.receipt_verified   # True
```

Этот адаптер передаёт `FunctionTool` **JSON Schema** хаба, а не pydantic-модель. С моделью
agent-framework 1.15.0 публикует схему по алиасам, а проверяет по ней же дамп без алиасов, —
поэтому capability, у которой аргумент называется `lambda` (такая есть: `fourier.verify@v1`),
вызвать невозможно вообще: `TypeError: Missing required argument(s): lambda`. Передача схемы
убирает этот круг целиком, и модель читает контракт хаба как есть.

Два следствия, о которых стоит знать. Отсутствующий обязательный аргумент и неверный
скалярный тип фреймворк отклоняет **до** вызова, так что ни то ни другое не стоит денег. А вот
свойство, которого нет в схеме, он не отклоняет — его отбрасывает сам адаптер: отправить
выдуманный аргумент значит заплатить за вызов, который capability откажется выполнять.
...
Адаптеры писались под crewai **1.15.8** и проходят на 1.15.9 — вот что здесь полезно. Две
версии pydantic выше — исторический факт: при повторном замере 2026-08-24 crewai 1.15.17,
autogen-core 0.7.5 и agent-framework-core 1.15.0 разрешаются вместе на pydantic 2.12.5, и CI
теперь гоняет все четыре набора — 582 теста — в одном окружении.
