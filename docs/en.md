...
tools inside LangChain/LangGraph, CrewAI, AutoGen and Microsoft Agent Framework.
...
flowchart TB
    subgraph consumer["Consumer process"]
        ...
        subgraph bridge["aimarket-bridges"]
            adapter["Framework adapter<br/>langchain.py · crewai.py<br/>autogen.py · agent_framework.py"]
...
| Adapters | `langchain.py`, `crewai.py`, `autogen.py`, `agent_framework.py` | one framework each |
...
flowchart TB
    ...
    cap -->|"schema as published"| af["agent-framework-core 1.15.0<br/>FunctionTool(input_model=schema)<br/>async invoke()"]
...
from documentation — all four had moved past what the docs implied.
...
**agent-framework-core 1.15.0** takes either a pydantic model or a raw JSON Schema — and only
the schema works here. Given a model it advertises pydantic's by-alias schema and then
validates the *un-aliased* dump against it, so `fourier.verify@v1` is uncallable through that
door: `TypeError: Missing required argument(s): lambda`. Handed the hub's schema it validates
required arguments and scalar types before the call (neither costs money) but forwards
undeclared properties, which the adapter drops itself. Sync tools are offloaded to
`asyncio.to_thread` — the loop's shared default executor — so the adapter passes a coroutine and
uses its own bounded pool instead.
...
The same trap is why the Agent Framework adapter refuses a pydantic model altogether:
agent-framework 1.15.0 advertises `model_json_schema()` (aliased, `lambda`) but validates
`model_dump(exclude_unset=True)` (un-aliased, `lambda_`) against it, so the round-trip is broken
inside the framework rather than in the bridge. There the fix is to hand over the hub's schema
and never build a model at all.
...
Install only the extra you use — this package keeps no framework in its own dependencies,
because their pydantic floors have collided before and a bridge has no business dragging one
framework into another's environment. Measured 2026-08-24: crewai 1.15.17, autogen-core 0.7.5
and agent-framework-core 1.15.0 do currently resolve together on pydantic 2.12.5, which is how
CI runs all four suites in one job.
...
Calling one directly — `invoke` is async and takes the arguments as a mapping:
...
contents[0].text                    # exactly what the model reads
tool.last_result.output             # the capability's own answer
tool.last_result.receipt_verified   # True
```

This adapter passes the hub's **JSON Schema** to `FunctionTool`, not a pydantic model. Given a
model, agent-framework 1.15.0 advertises the by-alias schema and then validates the un-aliased
dump against it, so a capability whose argument is called `lambda` — `fourier.verify@v1` is
one — can never be called: `TypeError: Missing required argument(s): lambda`. Handing over the
schema avoids the round-trip entirely and shows the model the hub's own contract.

Two consequences worth knowing. The framework refuses a missing required argument and a wrong
scalar type *before* the call, so neither costs money. It does **not** reject a property the
schema never declared — the adapter drops those itself, because sending an invented argument
means paying for a call the capability will refuse.
...
fact. The two pydantic lines above are historical: measured again on 2026-08-24, crewai 1.15.17,
autogen-core 0.7.5 and agent-framework-core 1.15.0 resolve together on pydantic 2.12.5, and CI
now runs all four suites — 582 tests — in one environment.
